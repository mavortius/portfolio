...
export class AccountService {
  ...
  purchase(stock: Stock): void {
    ...
    if (stock.price < this.balance) {
      ...
      You bought ${stock.symbol} for ${this.currencyPipe.transform(stock.price, 'USD', true, '.2')}`,
        'success');
    } else {
  ...
  sell(index: number): void {
    ...
    if (stock) {
      ...
      this.cacheValues();
      this.alertService.alert(
        `You sold ${stock.symbol} for ${this.currencyPipe.transform(stock.price, 'USD', true, '.2')}`,
        'success');
    } else {
